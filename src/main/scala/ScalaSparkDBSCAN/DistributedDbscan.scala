...
import ScalaSparkDBSCAN.dbscan.{ClusterId, PointId, RawDataSet, TempPointId}
import ScalaSparkDBSCAN.spatial._
import org.apache.commons.math3.ml.distance.DistanceMeasure

import scala.collection.mutable.{HashSet, ListBuffer}
import org.apache.spark.rdd.RDD
import ScalaSparkDBSCAN.spatial.rdd.{PartitioningSettings, PointsInAdjacentBoxesRDD, PointsPartitionedByBoxesRDD}

import scala.Some
import ScalaSparkDBSCAN.util.commandLine.CommonArgs
import ScalaSparkDBSCAN.util.debug.DebugHelper
...
/** Implementation of the DBSCAN algorithm which is capable of parallel processing of the input data.
  ...
  *  - Clustering data in each partition separately   --- local clustering
  *  - Merging clusters found in different partitions   --- global merging
  *
  * @param settings Parameters of the algorithm. See [[ScalaSparkDBSCAN.DbscanSettings]]
  *                 for details
  * @param partitioningSettings Partitioning settings. See [[ScalaSparkDBSCAN.spatial.rdd.PartitioningSettings]]
  *                             for details
  */
class DistributedDbscan (
  settings: DbscanSettings,
  partitioningSettings: PartitioningSettings = new PartitioningSettings ())
  extends Dbscan (settings, partitioningSettings) with DistanceCalculation with Logging with Serializable {

  private [ScalaSparkDBSCAN] implicit val distanceMeasure: DistanceMeasure = settings.distanceMeasure

  /** Runs the clustering algorithm
   *
   * @param data A data set to be clustered. See [[ScalaSparkDBSCAN.dbscan.RawDataSet]] for details
   * @return A [[ScalaSparkDBSCAN.DbscanModel]] object which represents clustering results
   */
  override protected def run(data: RawDataSet): DbscanModel = {
    val distanceAnalyzer = new DistanceAnalyzer (settings)
    // A. local partitioning, we get a Density-based partitioned RDD where each point is accompanied by its sort key
    // i.e. a key used to compare the boxes to which a point belongs
    ...
    // B. calculating neighbors of each point
    val pointsWithNeighborCounts = distanceAnalyzer.countNeighborsForEachPoint(partitionedData)
    //Broadcast variables allow the programmer to keep a read-only variable cached on each machine rather than shipping
    // a copy of it with tasks. They can be used, for example, to give every node a copy of a large input dataset
    // in an efficient manner. After the broadcast variable is created, it should be used instead of the value v in
    // any functions run on the cluster so that v is not shipped to the nodes more than once.
    // In addition, the object v should not be modified after it is broadcast in order to ensure that all nodes
  ...
  /** Finds clusters within one partition
    ...
    *           when this method is called, [[ScalaSparkDBSCAN.spatial.Point]] objects in the collection
    *           already contain all necessary metadata, including number of their neighbors
    *
    * @param boundingBox A [[ScalaSparkDBSCAN.spatial.Box]] object which represents bounds of the whole dataset
    ...
    */
  private [ScalaSparkDBSCAN] def findClustersInOnePartition(it: Iterator[(PointSortKey, Point)],
  ...
  /** Expands a cluster. In contrast to the function described in http://en.wikipedia.org/wiki/DBSCAN ,
    ...
    */
  private def expandCluster (points: Map[TempPointId, PartiallyMutablePoint],
                             index: PartitionIndex,
                             startingPoint: PartiallyMutablePoint,
                             settings: DbscanSettings): Unit = {
  ...
  /** Detects clusters in different partitions which should be merged together and generates mappings
    ...
    */
  private [ScalaSparkDBSCAN] def generateMappings (localData: Seq[Point]): (HashSet[(HashSet[ClusterId], ClusterId)],
  ...
  private [ScalaSparkDBSCAN] def reassignClusterId (
    pt: Point,
