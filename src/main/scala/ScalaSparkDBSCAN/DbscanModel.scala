...
import ScalaSparkDBSCAN.spatial.{DistanceAnalyzer, Point}
import ScalaSparkDBSCAN.spatial.rdd.PointsPartitionedByBoxesRDD

/** Represents results calculated by DBSCAN algorithm.
  *
  * This object is returned from [[ScalaSparkDBSCAN.Dbscan.run]] method.
  * You cannot instantiate it directly
  */
class DbscanModel private[ScalaSparkDBSCAN] (val allPoints: RDD[Point], val settings: DbscanSettings)
  ...
  /** Predicts which cluster a point would belong to
    *
    * @param newPoint A [[ScalaSparkDBSCAN.dbscan.PointCoordinates]] for which you want to make a prediction
    ...
    *         [[ScalaSparkDBSCAN.DbscanModel.NoisePoint]] is returned.
    *         If the point is surrounded by so many noise points that they can constitute a new
    *         cluster, then [[ScalaSparkDBSCAN.DbscanModel.NewCluster]] is returned
    */
...
/** Contains constants which designate cluster ID
  *
  */
object DbscanModel extends Serializable {
  ...
  /** Indicates that a new cluster would appear in a [[ScalaSparkDBSCAN.DbscanModel]] if
  ...
  /** Initial value for cluster ID of each point.
    *
    */
  private [ScalaSparkDBSCAN] val UndefinedCluster: ClusterId = -2
}
