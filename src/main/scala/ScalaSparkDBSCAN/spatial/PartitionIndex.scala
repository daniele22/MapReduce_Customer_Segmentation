...
import ScalaSparkDBSCAN.util.math.DoubleComparisonOperations._
...
/** An indexing data structure which allows fast lookup of point's neighbors
  ...
  */
private [ScalaSparkDBSCAN] class PartitionIndex (val partitionBounds: Box,
  ...
  private [ScalaSparkDBSCAN] val boxesTree = PartitionIndex.buildTree (partitionBounds, partitioningSettings, dbscanSettings)
  ...
  private [ScalaSparkDBSCAN] def findPotentiallyClosePoints (pt: Point): Iterable[Point] = {
  ...
  private [ScalaSparkDBSCAN] def findBoxForPoint (pt: Point, root: BoxTreeItemWithPoints): BoxTreeItemWithPoints = {
...
private [ScalaSparkDBSCAN] object PartitionIndex extends DistanceCalculation with Serializable {
