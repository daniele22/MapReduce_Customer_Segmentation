...
import ScalaSparkDBSCAN.dbscan.{BoxId, PairOfAdjacentBoxIds, RawDataSet}
import ScalaSparkDBSCAN.spatial.rdd.{BoxPartitioner, PartitioningSettings}
import org.apache.spark.rdd.RDD
import org.apache.spark.SparkContext._

/** Calculates box-shaped regions for density-based partitioning (see [[ScalaSparkDBSCAN.spatial.rdd.BoxPartitioner]] )
  * and for fast lookup of point's neighbors (see [[ScalaSparkDBSCAN.spatial.PartitionIndex]]
  *
  * @param data A raw data set
  */
private [ScalaSparkDBSCAN] class BoxCalculator (val data: RawDataSet) extends Serializable {
  ...
  private [ScalaSparkDBSCAN] def getNumberOfDimensions (ds: RawDataSet): Int = {
    val pt = ds.first()
  ...
  private [ScalaSparkDBSCAN] def calculateBounds (ds: RawDataSet, dimensions: Int): List[BoundsInOneDimension] = {
...
private [ScalaSparkDBSCAN] object BoxCalculator extends Serializable{
  ...
  private [ScalaSparkDBSCAN] def generateCombinationsOfSplits (splits: List[List[BoundsInOneDimension]],
  ...
  private [ScalaSparkDBSCAN] def assignAdjacentBoxes (boxesWithEnoughPoints: Iterable[Box]) = {
  ...
  private [ScalaSparkDBSCAN] def generateDistinctPairsOfAdjacentBoxIds (boxesWithAdjacentBoxes: Iterable[Box]): Iterable[PairOfAdjacentBoxIds] = {
  ...
  private [ScalaSparkDBSCAN] def shouldAdjacentBoxBeIncludedInPartition (rootBoxId: BoxId, adjacentBoxId: BoxId): Boolean = {
    rootBoxId <= adjacentBoxId
  }

  private [ScalaSparkDBSCAN] def generateEmbracingBox (boxes: Iterable[Box]): Box = {
  ...
  private [ScalaSparkDBSCAN] def generateEmbracingBoxFromAdjacentBoxes (rootBox: Box): Box = {
